...
public Action MenuOpen(int client, int args) {
	...
	if ((cvar = FindConVar("sm_bbr_enabled")) != null) {
		...
		Format(text, sizeof(text), "RCBots are robots [%s]", state);
...
public int Handle_Menu(Menu menu, MenuAction action, int client, int item) {
	if (action == MenuAction_Select) {
		...
		else if (StrEqual(info, "rcbots_robots"))
			Voting_CreateYesNoConVarVote(client, "sm_bbr_enabled", "Make RCBots robots?", 1, 0, "RCBots as robots");
		else if (StrEqual(info, "rcbots_cweps"))
