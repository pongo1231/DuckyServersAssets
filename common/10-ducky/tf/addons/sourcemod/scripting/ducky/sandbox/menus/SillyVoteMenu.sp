...
public Action MenuOpen(int client, int args) {
	...
	if ((cvar = FindConVar("mp_friendlyfire")) != null) {
		FormatToggle(cvar, state, sizeof(state));
		Format(text, sizeof(text), "Friendly fire [%s]", state);
		menu.AddItem("silly_friendly_fire", text);
	}

	if ((cvar = FindConVar("sm_deadlywater_enabled")) != null) {
		FormatToggle(cvar, state, sizeof(state));
		Format(text, sizeof(text), "Deadly water [%s]", state);
...
public int Handle_Menu(Menu menu, MenuAction action, int client, int item) {
	if (action == MenuAction_Select) {
		...
		else if (StrEqual(info, "silly_friendly_fire"))
			Voting_CreateYesNoConVarVote(client, "mp_friendlyfire", "Enable friendly fire?", 1, 0, "Friendly fire");
		else if (StrEqual(info, "silly_deadly_water"))
			Voting_CreateYesNoConVarVote(client, "sm_deadlywater_enabled", "Enable deadly water?", 1, 0, "Deadly water");
