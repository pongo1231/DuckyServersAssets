...
for id,v in data['items'].items():
	...
	try:
		...
		dbc.execute('INSERT INTO new_tf2idb_item '
			...
			'VALUES (?,?,?,?,?,?,?,?,?,?,?,?)', 
			(id,i['name'],i.get('item_name', ''),i.get('item_class', ''),i.get('item_slot', ''),i.get('item_quality', ''), tool, i.get('min_ilevel'), i.get('max_ilevel'),baseitem,
		...
		for capabilty in i.get('capabilities', []):
			dbc.execute('INSERT INTO new_tf2idb_capabilities (id,capability) VALUES (?,?)', (id, capabilty))
